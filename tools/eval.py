...
ROOTS = ("fixtures/synthetic", "fixtures/lexgap", "fixtures/real")
...
def discover_cases(roots=ROOTS):
    ...
    cases = []
    for root in roots:
        kind = ("lexical-gap" if "lexgap" in root
                else "real" if "real" in root else "synthetic")
...
def evaluate_case(case, kb_dir="kb", use_llm=True, use_embed=True):
    trace = load_trace(case["trace"])
    truth = case["truth"]
    cause = truth["cause_kb_id"]
    windows = find_hitches(trace)
    row = {"name": case["name"], "kind": case["kind"], "cause": cause,
           "detected": any(w.first_frame <= truth["hitch_frame"] <= w.last_frame for w in windows),
           "bm25_top1": None, "embed_top1": None,
           "llm_top1": None, "llm_top3": None, "top_hypothesis": None}
    if not windows:
        return row
    ...
    bm = retrieve(bundle, kb_dir=kb_dir)
    row["bm25_top1"] = bool(bm) and bm[0][0]["id"] == cause

    if use_embed:
        from pipeline.retrieve_embed import retrieve_embed
        em = retrieve_embed(bundle, kb_dir=kb_dir)
        row["embed_top1"] = bool(em) and em[0][0]["id"] == cause

    if use_llm:
        from pipeline.reason import reason
        hyps = reason(bundle, bm).get("hypotheses", [])
        kb_ids = [h.get("kb_id") for h in hyps]
        row["llm_top1"] = cause in kb_ids[:1]
        row["llm_top3"] = cause in kb_ids[:3]
        if hyps:
            row["top_hypothesis"] = hyps[0].get("kb_id") or hyps[0].get("cause")
    return row


def run_eval(kb_dir="kb", use_llm=True, use_embed=True):
    return [evaluate_case(c, kb_dir=kb_dir, use_llm=use_llm, use_embed=use_embed)
...
def _agg(rows, key, kinds=None):
    sub = rows if kinds is None else [r for r in rows if r["kind"] in kinds]
    return _rate(sub, key)
...
def to_markdown(rows):
    ...
    lines = [
        ...
        "LLM top-k is at temperature 0. BM25 vs embed = keyword vs semantic "
        "KB retrieval (nomic-embed-text).\n",
    ...
    for r in rows:
        ...
        lines.append(
            ...
            f"{_fmt(r['bm25_top1'])} | {_fmt(r['embed_top1'])} | "
            f"{_fmt(r['llm_top1'])} | {_fmt(r['llm_top3'])} | {top} |"
        )
    ...
    lines += [
        "",
        "## Aggregate\n",
        f"- detection recall: {pct(_agg(rows, 'detected'))} ({n} cases)",
        f"- BM25 rank-1: {pct(_agg(rows, 'bm25_top1'))} overall "
        f"| clean {pct(_agg(clean, 'bm25_top1'))} | lexical-gap {pct(_agg(lexgap, 'bm25_top1'))}",
        f"- embed rank-1: {pct(_agg(rows, 'embed_top1'))} overall "
        f"| clean {pct(_agg(clean, 'embed_top1'))} | lexical-gap {pct(_agg(lexgap, 'embed_top1'))}",
        f"- LLM top-1: {pct(_agg(rows, 'llm_top1'))} | top-3: {pct(_agg(rows, 'llm_top3'))}",
        "",
    ]
    return "\n".join(lines)
...
def to_csv(rows, path):
    cols = ["name", "kind", "cause", "detected", "bm25_top1", "embed_top1",
...
def main(argv=None):
    ...
    ap.add_argument("--no-llm", action="store_true", help="skip the LLM top-k columns")
    ap.add_argument("--no-embed", action="store_true", help="skip the embedding retriever column")
    ...
    rows = run_eval(kb_dir=args.kb, use_llm=not args.no_llm, use_embed=not args.no_embed)
    md = to_markdown(rows)
